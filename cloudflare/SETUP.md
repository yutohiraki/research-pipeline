...
## メモ
- **他人の論文は混ざりません**: 通知は「**bot ↔ 本人の1対1 DM**」に届く（各自の Mac が自分の `dm_user_id` に投稿）。
  タップも **Slackユーザー別キュー**（`q:{kind}:<自分のID>`）で、自分の Mac は自分のぶんだけ取得。
  → 共有しているのは **Worker と bot の"身元"だけ**で、**各自の論文リストは各自の DM に閉じています**。
  ⚠️ ただし `bot_token` は共有秘密（配った全員が持つ）＝**信頼できるメンバー内**で使う前提。完全分離したいなら各自が自分の Slack アプリ＋Worker を立てる（管理者パートを各自で）。
